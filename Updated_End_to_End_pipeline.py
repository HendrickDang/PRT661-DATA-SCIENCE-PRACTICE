...
def cv_rmse_for_features(model, cv_frame: pd.DataFrame, features: list[str]) -> float:
    ...
    X = cv_frame[features].astype(float).to_numpy()
...
def evaluate_feature_variants(train: pd.DataFrame,
    ...
    # TimeSeriesSplit needs calendar order; restrict to the training period.
    cv_train = train.sort_values(["date", "Region"]).reset_index(drop=True)

    variant_rmse: dict[str, float] = {}

    for name, features in variants.items():
        rmse = cv_rmse_for_features(LinearRegression(), cv_train, features)
        variant_rmse[name] = rmse
        print(f"   {name:<20} CV RMSE={rmse:.4f}")
    ...
    ax.set(
        ...
        title=("Feature Variant Comparison - Linear Regression\n"
               f"(TimeSeriesSplit CV, train {YEAR_MIN}-2022 | red = best)"),
    )
    fig.tight_layout()
...
def run_regression(monthly_assault: pd.DataFrame,
    ...
    FEATURES = evaluate_feature_variants(train, dummy_cols)
